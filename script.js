...
function loadBoletos(apartmentId) {
  ...
  // Cria a div para o carregamento do arquivo do boleto (mantemos essa criação)
  const loadingArquivoBoleto = document.createElement('div');
  loadingArquivoBoleto.id = 'loading-arquivo-boleto';
  loadingArquivoBoleto.style.display = 'none'; // Inicialmente escondido
        ...
        boletos.forEach(boleto => {
          if (boleto.name && boleto.fileId) {
            ...
            link.onclick = function(event) {
              ...
              // Substitui o conteúdo da lista pelo indicador de carregamento
              boletosListContainer.innerHTML = loadingArquivoBoleto.outerHTML;
              ...
              fetch(`/.netlify/functions/load-boletos-content?fileId=${fileId}`)
                ...
                .catch(error => {
                  ...
                  // Opcional: Aqui você poderia adicionar uma mensagem de erro de carregamento no lugar da imagem.
                });
            };
            listItem.appendChild(link);
